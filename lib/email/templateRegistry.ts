...
export const EMAIL_TEMPLATES: EmailTemplateDefinition[] = [
    ...
    },
    {
        ...
        label: "Golden ticket",
        whenSent:
            "When someone takes their ticket on their Golden program page, and from the Golden program console when an admin activates or sends one. Carries the code. The '_granted' wordings replace the others when the ticket is already on an existing account, where there is no code left to redeem; the console can also put a line of its own near the top and override the subject. It is the golden page in an inbox, pictures included: the price in the Activate card is Pro's yearly price from Paddle and the founders' cards are their newest posts, both read at send time; the pictures are captures of the page (scripts/capture-golden-email-assets.mjs).",
        keyPrefix: "golden_ticket",
        fields: [
            // In the order the email reads, top to bottom: the page's opening,
            // its four showcase sections, the benefits, founders, globe and
            // film, then the Activate card that holds the code.
            "subject",
            "preheader",
            "badge",
            "ticket_label",
        ...
        listFields: ["benefits"],
        variables: ["name", "code", "invites", "number"],
    },
    {
        id: "waitlist",
