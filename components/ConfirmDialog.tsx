...
export default function ConfirmDialog({
    ...
    const { swipeHandlers, swipeStyle } = useSheetSwipe(onCancel);

    if (!isOpen || typeof document === 'undefined') return null;

    return createPortal(
        <div
            className="sheet-shell fixed inset-0 bg-stone-900/40 backdrop-blur-xs z-[100] flex items-center justify-center p-4 animate-in fade-in duration-200"
            onClick={onCancel}
        >
            <div
                className="sheet-panel bg-white rounded-[24px] border border-stone-200/80 shadow-[0_20px_50px_rgba(0,0,0,0.12)] max-w-md w-full p-8 flex flex-col gap-6 animate-in zoom-in-95 duration-200 text-center" {...swipeHandlers} style={swipeStyle}
            ...
            >
                ...
                <div className="sheet-panel-footer flex items-center justify-center gap-4 mt-2">
                    <button
                        onClick={onCancel}
